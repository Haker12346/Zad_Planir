...
namespace PlanZad
{
    public partial class FormRedS : MaterialForm
    {
        ...
        private void Form2_Load(object sender, EventArgs e)
        {
            ...
            FormStat frm = (FormStat)Application.OpenForms["FormStat"];
            int row = frm.dataGridViewS.CurrentRow.Index;
            TbI.Text = Convert.ToString(frm.dataGridViewS[0, row].Value);
            TbN.Text = Convert.ToString(frm.dataGridViewS[1, row].Value);
        }
        ...
        private void buttoRed_Click(object sender, EventArgs e)
        {
...

            {
                using (var connection = new SqlConnection(get_cs()))
                {
                    ...
                    using (var cmd = new SqlCommand("Update Status set StatusID = @StatusID,Naimenovanie = @Naimenovanie where StatusID = @StatusID", connection))
